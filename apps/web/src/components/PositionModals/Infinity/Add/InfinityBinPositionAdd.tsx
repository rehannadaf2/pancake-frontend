...
export const InfinityBinPositionAdd = ({ position, poolInfo }: InfinityBinPositionAddProps) => {
  ...
  const priceRangeProps = useMemo(() => {
    if (!currentPrice || !positionMinPrice || !positionMaxPrice) {
      return { minPrice: '-', maxPrice: '-', minPriceRaw: 0, maxPriceRaw: 0, currentPriceRaw: 0 }
    }
  ...
  const minBinId = useMemo(() => (pool?.activeId ? pool.activeId - MAX_BIN_NUM_PER_SIDE : null), [pool?.activeId])
  const maxBinId = useMemo(() => (pool?.activeId ? pool.activeId + MAX_BIN_NUM_PER_SIDE : null), [pool?.activeId])
